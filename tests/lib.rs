...
fn check_haplotype_fractions_5050() {
    ...
    let _ = orthanq::calling::haplotypes::CallerBuilder::default()
        .hdf5_reader(hdf5::File::open("tests/Sample_HLA00601-0.5_HLA21879-0.5_DQA1_abundance.h5").unwrap())
        .haplotype_variants(bcf::Reader::from_path("tests/hla-allele-variants_v4.vcf.gz").unwrap())
        .haplotype_calls(bcf::Reader::from_path("tests/Sample_HLA00601-0.5_HLA21879-0.5.bcf").unwrap())
        .observations(bcf::Reader::from_path("tests/Sample_HLA00601-0.5_HLA21879-0.5_obs.bcf").unwrap())
        .min_norm_counts(0.01)
        .max_haplotypes(2)
        .use_evidence("both".to_string())
        .outcsv(Some(output))
    ...
    let headers = rdr.headers().unwrap();
    assert_eq!(vec![&headers[2], &headers[3]], vec!["HLA:HLA00601", "HLA:HLA21879"]); //both are from locus DQA1.
